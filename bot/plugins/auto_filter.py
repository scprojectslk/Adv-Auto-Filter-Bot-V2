...
async def auto_filter(bot, update):
    ...
    for filter in filters: # iterating through each files
        ...
        if file_size < 1024:
            ...
            file_size = f'[{round(file_size/1024, 2)} KiB] '
        elif file_size < (1024**3):
            file_size = f'[{round(file_size/(1024**2), 2)} MiB] '
        elif file_size < (1024**4):
            file_size = f'[{round(file_size/(1024**3), 2)} GiB] '
    ...
    result[0].append(
        [
            InlineKeyboardButton(
                f'🔰 Page 1/{min(len_result, max_pages)} 🔰',
